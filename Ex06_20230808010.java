...
/* 
  ...
  */      
    ...
    public static char[] grade(int[] students) {
        ...
        // Your code goes here...
        int highScore=getMax(students);
        char[] letterGrade=new char[students.length];
        for (int i = 0; i < letterGrade.length; i++) {
            if(students[i]>=(highScore-10)){
                letterGrade[i]='A';
            }
            else if(students[i]>=(highScore-20)){
                letterGrade[i]='B';
            }
            else if(students[i]>=(highScore-30)){
                letterGrade[i]='C';
            }        
            else if(students[i]>=(highScore-40)){
                letterGrade[i]='D';
            }       
            else {
                letterGrade[i]='F';
  ...
  public static int random(int start, int end, int... exclude) {
        ...
        for (int i = 0; i < exclude.length; i++) {
            for (int j = 0; j < exclude.length; j++) {
                if(result==exclude[j]){
                    result= start + (int)(Math.random()*(end-start));
                }
            }
        }
         return result;
